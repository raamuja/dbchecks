...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  @media (prefers-reduced-motion: reduce){
    .theme-toggle .theme-pointer{ animation:none; }
  }
  .theme-btn{
  ...
  @media (prefers-reduced-motion: reduce){
    .dash-filter .filter-pointer{ animation:none; }
  }
  .select-glow-wrap{
  ...
  .section-title-box h2.ase-summary-title{
    color:#0f766e;
    text-shadow:0 1px 0 rgba(255,255,255,0.7), 0 2px 4px rgba(15,118,110,0.22);
  }
  .section-title-box h2.rep-summary-title{
    color:#b45309;
    text-shadow:0 1px 0 rgba(255,255,255,0.7), 0 2px 4px rgba(180,83,9,0.22);
  }
  .section-title-box h2.iq-summary-title{
    color:#6d28d9;
    text-shadow:0 1px 0 rgba(255,255,255,0.7), 0 2px 4px rgba(109,40,217,0.22);
  }
  body.theme-dark .section-title-box h2.ase-summary-title,
  body.theme-dark .section-title-box h2.rep-summary-title,
  body.theme-dark .section-title-box h2.iq-summary-title{
    text-shadow:0 1px 0 rgba(0,0,0,0.5), 0 2px 4px rgba(0,0,0,0.4);
  }
  .wide-header-wrap{
    margin-top:12px;
    margin-bottom:6px;
  ...
  .summary-dbname.dbname-critical .dbname-text{
    display:inline-block;
    color:#ffffff;
    font-weight:800;
    background:#3d0000;
  ...
  @media (prefers-reduced-motion: reduce){
    .summary-dbname.dbname-critical .dbname-text{ animation:none; }
  }
  .top-tables-btn{
    display:block;
    margin:4px auto 0;
  ...
  .top-tables-btn:hover{ background:#ffecec; }
  .top-tables-btn.active{ background:#ffdede; }
  body.theme-dark .top-tables-btn{
    background:#3a1414;
    color:#ff3d3d;
    border-color:#c24545;
    text-shadow:0 0 8px rgba(255,70,70,0.85);
  }
  body.theme-dark .top-tables-btn:hover{ background:#4a1a1a; }
  body.theme-dark .top-tables-btn.active{ background:#521c1c; }
  .top-tables-btn .top-tables-label{
    display:inline-block;
    animation:pointer-blink 0.6s ease-in-out infinite;
  }
  @media (prefers-reduced-motion: reduce){
    .top-tables-btn .top-tables-label{ animation:none; }
  }

  /* Floating popover (a single shared element positioned via JS, so it can
     never be clipped by a card's overflow:hidden and never truncates). */
  .top-tables-popover{
    position:fixed;
    z-index:9999;
    display:none;
    ...
    border-radius:8px;
    box-shadow:0 8px 24px rgba(20,25,35,0.22);
    font-family:var(--mono);
    font-size:12px;
  }
  .top-tables-popover.open{ display:block; }
  .top-tables-popover .ttp-header{
    display:grid;
  ...
  .top-tables-popover .ttp-divider{
    height:2px;
    background:#8b95a5;
  }
  .top-tables-popover .ttp-row{
    display:grid;
    grid-template-columns:200px auto;
    gap:14px;
    padding:6px 0;
  ...
  .top-tables-popover .ttp-row:last-child{ border-bottom:none; }
  .top-tables-popover .ttp-row:hover{ background:#f3f5f8; }
  .top-tables-popover .ttp-name{
    overflow-wrap:anywhere;
    padding:0 12px 0 12px;
    border-right:2px solid #8b95a5;
  }
  .top-tables-popover .ttp-count{ white-space:nowrap; color:#111827; padding:0 12px 0 0; }
  body.theme-dark .top-tables-popover{
    background:#1a2332;
    border-color:#3a4a63;
    box-shadow:0 8px 24px rgba(0,0,0,0.5);
  }
  body.theme-dark .top-tables-popover .ttp-header{
    background:#0f1522;
    color:#f5f7fa;
  }
  body.theme-dark .top-tables-popover .ttp-divider{
    background:#7d8aa0;
  }
  body.theme-dark .top-tables-popover .ttp-row{
    color:#e5e9f0;
    border-bottom-color:#2c3a51;
  }
  body.theme-dark .top-tables-popover .ttp-row:hover{ background:#22304a; }
  body.theme-dark .top-tables-popover .ttp-count{ color:#f5f7fa; }
  body.theme-dark .top-tables-popover .ttp-name{ border-right-color:#7d8aa0; }
  .summary-slno{
    text-align:center;
    color:var(--text-dim);
    font-weight:600;
  ...
  .trend-badge.trend-up{ color:#b45309; }
  .trend-badge.trend-down{ color:#15803d; }
  ...
  body.theme-dark .summary-dbname.dbname-critical .dbname-text{
    color:#ffffff;
    background:#ff3d3d;
    box-shadow:0 0 10px rgba(255,61,61,0.75);
  }
  body.theme-dark .trend-badge.trend-up{ color:#fbbf24; }
  body.theme-dark .trend-badge.trend-down{ color:#4ade80; }
...
<header class="top-bar">
  <div class="page-title-box">
    ...
    <div class="theme-toggle" role="group" aria-label="Theme">
      ...
      <button type="button" id="themeLightBtn" class="theme-btn active" title="Light Theme" aria-label="Light Theme" onclick="setTheme('light')"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="#f59e0b" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="4.2"/><path d="M12 2.5v2.6M12 18.9v2.6M4.55 4.55l1.85 1.85M17.6 17.6l1.85 1.85M2.5 12h2.6M18.9 12h2.6M4.55 19.45l1.85-1.85M17.6 6.4l1.85-1.85"/></svg></button>
      <button type="button" id="themeDarkBtn" class="theme-btn" title="Dark Theme" aria-label="Dark Theme" onclick="setTheme('dark')"><svg viewBox="0 0 24 24" width="16" height="16" fill="#facc15" stroke="none"><path d="M20.6 15.1A8.9 8.9 0 1 1 9.4 3.4a7 7 0 0 0 11.2 11.7z"/></svg></button>
    </div>
  </div>
</header>
...
<div id="section-ase" class="wide-header-wrap">
  <div class="section-title-box wide-header-box ase-header-box">
    <h2 class="ase-summary-title">ASE - Database Status by Server</h2>
    <div class="legend">
...
<div id="section-replication" class="wide-header-wrap">
  <div class="section-title-box wide-header-box rep-header-box">
    <h2 class="rep-summary-title">Replication Status by Server</h2>
...
<div id="section-iq" class="wide-header-wrap">
  <div class="section-title-box wide-header-box iq-header-box">
    <h2 class="iq-summary-title">IQ Status by Server</h2>
...
// ---- Sample "Top 10 Tables" data for critical Data Size rows ----
// NOTE: There is no live database connection here. These table names and
// record counts are illustrative placeholders generated on the fly so the
// dropdown has something realistic to show. Swap this out once the real
// input file (actual top-table extracts) is available.
const SAMPLE_TABLE_POOL = [
  "CUSTOMER_MASTER","TRANSACTION_HISTORY","ORDER_DETAILS","ACCOUNT_LEDGER",
  "PAYMENT_RECORDS","AUDIT_LOG","INVOICE_DATA","PRODUCT_CATALOG",
  "SHIPMENT_TRACKING","VENDOR_MASTER","EMPLOYEE_RECORDS","SESSION_LOG",
  "ERROR_LOG","NOTIFICATION_QUEUE","SUBSCRIPTION_DATA","INVENTORY_SNAPSHOT",
  "PRICING_HISTORY","USER_PROFILE","CONTRACT_DETAILS","CLAIM_RECORDS",
  "POLICY_MASTER","STAGING_DATA","ARCHIVE_DATA","EVENT_LOG",
  "BATCH_JOB_HISTORY","RECONCILIATION_DATA","SETTLEMENT_RECORDS",
  "MESSAGE_QUEUE_LOG","TRANSACTION_STAGING","GL_JOURNAL_ENTRIES"
];
...
function generateTopTables(seedStr){
  const rand = mulberry32(hashSeed(seedStr));
  const pool = SAMPLE_TABLE_POOL.slice();
  for (let i = pool.length - 1; i > 0; i--){
    const j = Math.floor(rand() * (i + 1));
    [pool[i], pool[j]] = [pool[j], pool[i]];
  }
  const picked = pool.slice(0, 10);
  let current = 800000 + Math.floor(rand() * 1200000); // top table: 8L - 20L rows
  const rows = picked.map(name => {
    const count = Math.max(500, Math.floor(current));
    current = current * (0.55 + rand() * 0.25); // natural decreasing curve
    return { name, count };
  });
  rows.sort((a, b) => b.count - a.count); // highest record count first, lowest last
  return rows;
}
...
function renderTopTablesDropdown(seedStr){
  return `<button type="button" class="top-tables-btn" data-ttp-seed="${escAttr(seedStr)}" onclick="toggleTopTablesPopover(this)"><span class="top-tables-label">Top 10</span> &#9662;</button>`;
}

// ---- Shared floating popover for the Top 10 Tables button ----
let ttpEl = null;
let ttpOpenBtn = null;
function ensureTopTablesPopover(){
  if (ttpEl) return ttpEl;
  ttpEl = document.createElement("div");
  ttpEl.className = "top-tables-popover";
  document.body.appendChild(ttpEl);
  return ttpEl;
}
function closeTopTablesPopover(){
  if (ttpEl) ttpEl.classList.remove("open");
  if (ttpOpenBtn) ttpOpenBtn.classList.remove("active");
  ttpOpenBtn = null;
}
function toggleTopTablesPopover(btn){
  const popover = ensureTopTablesPopover();
  if (ttpOpenBtn === btn){ closeTopTablesPopover(); return; }

  const seedStr = btn.getAttribute("data-ttp-seed") || "";
  const rows = generateTopTables(seedStr);
  const rowsHtml = rows.map(t => `
    <div class="ttp-row">
      <span class="ttp-name">${escHtml(t.name)}</span>
      <span class="ttp-count">${fmt(t.count)}</span>
    </div>`).join("");
  popover.innerHTML = `
    <div class="ttp-header"><span class="ttp-name">Table Name</span><span class="ttp-count">Record Count</span></div>
  ...
  positionTopTablesPopover(btn);
}
function positionTopTablesPopover(btn){
  if (!ttpEl) return;
  const r = btn.getBoundingClientRect();
  const vw = window.innerWidth, vh = window.innerHeight;
  const popW = Math.max(ttpEl.offsetWidth, 260);
  const popH = ttpEl.offsetHeight;
  let left = r.left;
  if (left + popW > vw - 8) left = Math.max(8, vw - popW - 8);
  let top = r.bottom + 4;
  if (top + popH > vh - 8) top = Math.max(8, r.top - popH - 4);
  ttpEl.style.left = left + "px";
  ttpEl.style.top = top + "px";
}
document.addEventListener("click", (e) => {
  if (!ttpOpenBtn) return;
  if (ttpOpenBtn.contains(e.target) || (ttpEl && ttpEl.contains(e.target))) return;
  closeTopTablesPopover();
});
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape") closeTopTablesPopover();
});
window.addEventListener("scroll", () => { if (ttpOpenBtn) positionTopTablesPopover(ttpOpenBtn); }, true);
window.addEventListener("resize", () => { if (ttpOpenBtn) positionTopTablesPopover(ttpOpenBtn); });
function fmt(n){ return n.toLocaleString("en-IN"); }
function pctFmt(p){ return (Math.round(p*100)/100).toFixed(1) + "%"; }
function trendBadge(delta){
  // delta is percentage points changed since the previous check, or
  // null/undefined when there's no prior run to compare against yet.
  ...
  return `<span class="trend-badge ${cls}" title="Change since previous check">${arrow} ${sign}${delta.toFixed(1)}%</span>`;
}
      ...
      <tbody>
        ${sortedRows.map((r, idx) => `
          <tr data-status="${combinedStatus(r)}" title="Data: ${fmt(r.dbUsed)}/${fmt(r.dbAlloc)} MB used (${pctFmt(r.dbPct)}) &#10; Log: ${fmt(r.logUsed)}/${fmt(r.logAlloc)} MB used (${pctFmt(r.logPct)}) &#10; Checked: ${GENERATED_AT}">
            ...
            <td class="summary-dbname${r.dbStatus === "critical" ? " dbname-critical" : ""}">
              <span class="dbname-text">${r.name}</span>
              ${r.dbStatus === "critical" ? renderTopTablesDropdown(server.name + "|" + r.name) : ""}
            </td>
            <td class="summary-pctcell ${r.dbStatus}">${pctFmt(r.dbPct)}${trendBadge(r.deltaData)}</td>
            <td class="summary-pctcell ${r.logStatus}">${pctFmt(r.logPct)}${trendBadge(r.deltaLog)}</td>
...
// Colors the closed dropdown box itself to match the selected category.
// (Browsers don't apply an <option>'s own color/background to the closed
// select display, only inside the open list, so the box otherwise stays
// stuck on the default teal text for every choice.)
const FILTER_COLORS = {
  all: { color:"#1f2937", background:"#ffffff" },
  critical: { color:"#8B0000", background:"#ffffff" },           // Dark Red
  warning: { color:"#C2410C", background:"#ffffff" },            // Dark Orange
  healthy: { color:"#15803d", background:"#ffffff" },            // Dark Green
  "critical-warning": { color:"#7c2d12", background:"#ffffff" },
  ase: { color:"#0f766e", background:"#ffffff" },
  replication: { color:"#713f12", background:"#ffffff" },
  iq: { color:"#6d28d9", background:"#ffffff" },
};
function updateFilterColor(value){
  const sel = document.getElementById("dashFilter");
  if (!sel) return;
  const c = FILTER_COLORS[value] || FILTER_COLORS.all;
  sel.style.color = c.color;
  sel.style.background = c.background;
}

function applyFilter(value){
  updateFilterColor(value);
  const sectionGroups = {
  ...
  // Filler rows only exist to keep cards in the same visual row equal height
  // when every row is shown. Once a status filter trims rows unevenly, those
  // fillers just add blank space below the real data, so hide them whenever
  // a specific status filter is active.
  document.querySelectorAll("tr.filler-row").forEach(tr => {
    tr.style.display = (statuses !== null) ? "none" : "";
  });

  // Per card: show the table normally if any row matches the active filter.
  // A card with nothing matching (e.g. an all-healthy replication server
  // while viewing "Critical & Warning" or "Only Critical") simply disappears
  // rather than showing an unrelated "all healthy" note — the note only made
  // sense for a view with no status filtering at all, which no longer applies
  // to any of the dropdown options here.
...
# ---- Trend history (Data/Log size % since the previous check) ----
# A small JSON snapshot is kept alongside the output HTML so each run can
# show "since last check" deltas without needing a database of its own.
# Missing/corrupt history is treated as "no prior data" rather than an
# error, so a fresh install or a wiped file never breaks report generation.
HISTORY_FILENAME = ".dbsize_history.json"


def _pct(used, alloc):
    return (used / alloc) * 100 if alloc else 0.0
...
def save_history(path, data):
    try:
        ...
        print(f"WARNING: could not save trend history ({e})", file=sys.stderr)


def apply_trend(sections, history):
    """Attach deltaData / deltaLog (percentage-point change since the
    previous run) to every ASE row in-place, and return the fresh snapshot
    that should be saved as this run's history for next time.
    """
    new_history = {}
    for server in sections:
        for row in server["rows"]:
            key = f'{server["name"]}||{row["name"]}'
            cur_db_pct = _pct(row.get("dbUsed", 0), row.get("dbAlloc", 0))
            cur_log_pct = _pct(row.get("logUsed", 0), row.get("logAlloc", 0))
            prev = history.get(key)
            if prev:
                row["deltaData"] = round(cur_db_pct - prev.get("dbPct", cur_db_pct), 1)
                row["deltaLog"] = round(cur_log_pct - prev.get("logPct", cur_log_pct), 1)
            else:
                row["deltaData"] = None
                row["deltaLog"] = None
            new_history[key] = {"dbPct": round(cur_db_pct, 2), "logPct": round(cur_log_pct, 2)}
    return new_history
...
def main():
    ...
    new_history = apply_trend(sections, history)
